...
export async function POST(request: Request) {
  try {
    ...
    if (totalChars > env.MAX_PROMPT_CHARS) return NextResponse.json({ error: "Prompt is too large." }, { status: 413 });
    ...
    const modelMessages: ModelMessage[] = parsed.messages.map((message) => ({ role: message.role, content: message.content }));
